...
export async function PATCH(req: NextRequest, { params }: { params: { commentId: string; userId: string } }) {
...
export async function DELETE(req: NextRequest, { params }: { params: { commentId: string; userId: string } }) {
    const { commentId, userId } = params;
    const backendRes = await fetch(`${BACKEND_URL}/comments/${commentId}/${userId}`, {
        method: "DELETE",
        headers: { "Content-Type": "application/json", cookie: req.headers.get("cookie") || "" },
