...
async function proxyReviewDislike(
    reviewId: string,
    method: "POST" | "DELETE",
    auth: NonNullable<Awaited<ReturnType<typeof getAuthorizedContext>>>
) {
    const response = await fetch(`${BASE_API_URL}/review-like/${reviewId}/dislike`, {
        method,
        headers: {
    ...
    if (!response.ok) {
        return NextResponse.json(
            { message: (payload as { message?: string }).message ?? "Unable to update review dislike." },
...
export async function POST(
    ...
    if (!auth) {
        return NextResponse.json({ message: "Please log in to dislike reviews." }, { status: 401 });
    }
    ...
    return proxyReviewDislike(reviewId, "POST", auth);
}
...
export async function DELETE(
    ...
    return proxyReviewDislike(reviewId, "DELETE", auth);
}
