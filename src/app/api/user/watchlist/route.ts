...
// async function createSeriesWatchlist(seriesId: string, auth: NonNullable<Awaited<ReturnType<typeof getAuthorizedContext>>>) {
//     const response = await fetch(`${BASE_API_URL}/watchlists/series`, {
//         method: "POST",
//         headers: {
//             "Content-Type": "application/json",
//             Authorization: `Bearer ${auth.accessToken}`,
...
async function createMovieWatchlist(
    movieId: string,
...
async function createSeriesWatchlist(
    seriesId: string,
    auth: NonNullable<Awaited<ReturnType<typeof getAuthorizedContext>>>
) {
    const response = await fetch(`${BASE_API_URL}/watchlists/series`, {
...
async function resolveWatchlistIdByMovieId(
    movieId: string,
    auth: NonNullable<Awaited<ReturnType<typeof getAuthorizedContext>>>
) {
    const response = await fetch(`${BASE_API_URL}/watchlists`, {
    ...
    const payload = (await response.json().catch(() => ({}))) as {
        result?: Array<{ id?: string; movieId?: string; seriesId?: string }>;
        data?: Array<{ id?: string; movieId?: string; seriesId?: string }>;
    };
    ...
    const match = list.find((item) => item.movieId === movieId || item.seriesId === movieId);
    return typeof match?.id === "string" ? match.id : null;
}
...
// ── POST: handles both movie and series ──────────────────────────────────────
export async function POST(request: NextRequest) {
    const auth = await getAuthorizedContext();

    if (!auth) {
        return NextResponse.json({ message: "Please log in to save to watchlist." }, { status: 401 });
    }
    ...
    // If seriesId is explicitly provided, use the series endpoint
    if (seriesId) {
        return createSeriesWatchlist(seriesId, auth);
    }

    // Otherwise fall back to movie endpoint
    if (movieId) {
        return createMovieWatchlist(movieId, auth);
    }

    return NextResponse.json({ message: "Movie ID or Series ID is required." }, { status: 400 });
}

// ── DELETE: works for both (watchlist ID is the same regardless of type) ─────
export async function DELETE(request: NextRequest) {
    const auth = await getAuthorizedContext();
    ...
    const watchlistId =
        watchlistIdFromBody ||
        (movieId ? await resolveWatchlistIdByMovieId(movieId, auth) : null);
