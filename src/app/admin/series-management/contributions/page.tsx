...
import { SeriesContributionTableClient } from "@/components/admin/series-contribution-table-client";
import {
    approveSeriesContribution,
    getAdminSeriesContributions,
    rejectSeriesContribution,
} from "@/service/admin-series-contribution.services";
...
function normalizeContributions(raw: unknown): ContributionRow[] {
    const list = extractArray(raw, ["contributions", "items", "result", "data", "series"]);
    ...
    return list
        .map((item, index) => {
            ...
            return {
                id: pickString(item, ["id", "_id", "contributionId", "seriesId"], `contribution-${index + 1}`),
                seriesTitle: pickString(item, ["seriesTitle", "title", "name"], "Untitled series"),
                contributorName,
                contributorEmail,
...
async function approveContributionAction(formData: FormData) {
    ...
    try {
        await approveSeriesContribution(contributionId);
    } catch (error) {
        console.error("Failed to approve series contribution:", error);
    }

    revalidatePath("/admin/series-management/contributions");
}
...
async function rejectContributionAction(formData: FormData) {
    ...
    try {
        await rejectSeriesContribution(contributionId);
    } catch (error) {
        console.error("Failed to reject series contribution:", error);
    }
...
export default async function AdminSeriesContributionsPage() {
    const rawContributions = await getAdminSeriesContributions().catch(() => []);
                            ...
                            <section className="overflow-hidden rounded-3xl border border-slate-200/80 bg-white/90 shadow-[0_10px_35px_rgba(15,23,42,0.07)] backdrop-blur">
                                <div className="border-b border-slate-100 bg-[linear-gradient(120deg,#0f172a_0%,#1e293b_45%,#334155_100%)] p-6 text-white sm:p-7">
                                    <div className="flex flex-wrap items-start justify-between gap-5">
                                        <div>
                                            <p className="text-sm text-slate-300">Admin / Series Management / Contributions</p>
                                            <h1 className="mt-1 text-3xl font-bold tracking-tight sm:text-4xl">Series Contribution Handler</h1>
                                            <p className="mt-2 max-w-2xl text-sm text-slate-300">Review incoming series submissions, inspect details, and approve or reject with confidence.</p>
                                        </div>
